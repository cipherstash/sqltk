...
mod test {
    ...
    #[test]
    fn can_associate_data_with_ast_node() {
        let mut store = AnnotationStore::<u8>::new();
        let string = String::from("HELLO");

        store.insert((&string).into(), 123);

        assert_eq!(Some(&123), store.get(&(&string).into()))
    }

    #[test]
    fn can_associate_data_with_any_ast_node_type_in_same_store() {
        let mut store = AnnotationStore::<u8>::new();
        let some_string = String::from("HELLO");
        let some_bool = true;

        store.insert((&some_string).into(), 100);
        store.insert((&some_bool).into(), 200);

        assert_eq!(Some(&100), store.get(&(&some_string).into()));
        assert_eq!(Some(&200), store.get(&(&some_bool).into()));
        assert_eq!(Some(&200), store.get(&(&some_bool).into()));
    }

    #[test]
    fn ast_cannot_be_dropped_while_annotations_exist() {
        let mut store = AnnotationStore::<u8>::new();
        let some_string = String::from("HELLO");
