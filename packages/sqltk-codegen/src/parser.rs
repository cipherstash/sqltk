...
impl SqlParserAstAnalyser {
        ...
        for item in items {
            match item {
                ...
                Item::Mod(ItemMod {
                    ...
                    self.parse_mod_recursively(true, items, path);
                    path.pop();
                }
                Item::Mod(ItemMod {
                    content: Some((_, items)),
                    ...
                    path.push(ident.clone());
                    self.parse_mod_recursively(false, items, path);
                    path.pop();
                }
                Item::Impl(ItemImpl {
    ...
    fn insert_public_type(&mut self, ident: &Ident, type_path: PubTypePath) {
        match self.public_types.insert(ident.clone(), type_path) {
            None => {}
            Some(_) => panic!("Public type {:?} already recorded", ident),
        }
    }
